# 彩花向けプロジェクト報告

## yutakaeng 配線図PDF検証アプリ

報告日: 2026-08-27  
対象: `FieldRiseJapan/FieldRise` / `yutakaeng_windows_validation`  
最新コミット: [`a30c43e`](https://github.com/FieldRiseJapan/FieldRise/commit/a30c43e)  
Windows Actions: run [`33078429531`](https://github.com/FieldRiseJapan/FieldRise/actions/runs/33078429531)（成功）

## 1. プロジェクトの目的

yutakaengは、文字選択できない配線図PDFから、ホットマーカー作成に必要な情報を抽出し、確認用Excelへ変換するWindows持ち運びアプリである。対象は、枠内の主文字、左右の接続先、線コード、枠直上の見出し、ページ・枠・行の原図照合情報である。PDF全体を文字として無理に変換するのではなく、図面の構造に合わせてセルと接続先を分離して読み取る。

最優先の品質方針は、正解率を見かけ上高くすることではなく、**不確かな文字列をExcelへ確定出力しないこと**である。読めない候補、候補不一致、先頭欠落、端子罫線混入、無意味な英字列は空欄または警告として残し、利用者が全行確認して警告0件にするまで最終ホットマーカー用データとは扱わない。

## 2. 現在実装されている機能

| 項目 | 現在の仕様 |
|---|---|
| 入力 | 1つの複数ページPDFをドラッグ&ドロップ、または選択ダイアログから指定 |
| OCR | PyMuPDFの300dpi描画画像、RapidOCR 3.9.2 + ONNX Runtimeのローカル認識専用モード |
| 主文字 | 枠内中央セルを広狭2通りで読取り、候補一致・信頼度・許容文字・業務上のゲートで安全判定 |
| 左右接続先 | 主文字枠の外側を左・右に分離して読取り、隣接列の混入を抑制 |
| 見出し | 枠直上の狭い帯を読取り、YF/OF/SF/BSF/RJF/TF/PF/RPF/LF/RF/FF/ZF/WF/IF/BTF/BZF/BWF等を分類 |
| 除外 | DT系、D-開始線コード、端子台接続先単独Iを対象外とし、ZTのLEFT/RIGHTは残す |
| Excel | A列から「マーク主文字、マーク個数、読取状態、確認状態、L側接続先、R側接続先、線コード」を固定配置 |
| マーク個数 | 主文字を安全に読めた別電線1本につき2。主文字未読取は0 |
| 保留 | 見出し・電線サイズを分類できない行はサイズ別シートへ混ぜず「警告一覧」へ隔離 |
| 持ち運び | `yutakaeng.exe`、Tesseract、RapidOCR、ONNX Runtime、モデル、説明書を同一フォルダへ同梱 |

## 3. 今回追加した改善

### 処理中画面

PDFをドロップした後は、ドロップ画面と選択ボタンを一時的に隠し、コード風のライブ処理ログを画面の大部分に表示する。ページ総数を解析開始直後に通知し、画面にはページ進捗、経過時間、推定残り時間を表示する。タイマーは250ミリ秒ごとに更新し、ページ処理の実測時間を平滑化して推定値を更新する。処理完了時だけ残り時間を00:00にするため、処理途中でカウントダウンが停止したように見えない設計にした。

### 警告セル限定のCopilot補助

ローカルOCRと安全ゲートの後も警告が残った行だけ、外部支援用の小画像を専用フォルダへ保存する。正常行、別ページ、PDF全体は外部支援用フォルダへ保存しない。解析完了後、警告画像が存在する場合だけ「Copilotへ警告セルを確認依頼」ボタンを表示する。

ボタンを押した場合も、利用者が警告画像を1件だけ選び、画像を確認してからCopilotのページを開く。画像の添付は利用者が手動で行い、アプリがPDF全体や複数行を自動送信することはない。Copilotの回答は補助候補であり、原図照合なしにExcelへ確定反映しない。外部送信を望まない場合はボタンを使わず、完全オフライン運用を継続できる。

Microsoft公式情報では、Microsoft 365 Copilot APIsを使う方式にはMicrosoft Entra IDのアプリ登録、OAuth、組織側の権限同意、対象ユーザーのライセンス等が必要とされている。[1] Copilot Studioでは画像やPDFのアップロード解析が可能だが、エージェント設定と利用チャネルの準備が必要である。[2] そのため現版は、秘密鍵をアプリへ埋め込まない手動確認方式を採用している。

## 4. UIブラッシュアップとExcel運用変更

解析完了時は、画面中央に大きな緑色の`解析完了！`を表示する。完了ダイアログにもExcelの保存先を表示し、処理終了とファイル出力完了を見落としにくくした。解析中の画面は`LIVE PIPELINE / LOCAL OCR / NO CLOUD UPLOAD`を見出しにし、コード風ログの文字サイズ、コントラスト、余白を強化した。開始時にはローカル処理・外部送信なし・警告行のみ確認というモードをログに表示する。

Excelの`マーク主文字`は、読取できた場合に`盤番号-見出しまたは端子台-主文字`の順で出力する。例えば`1A-YF-FL1A11`、`3A-T1-BTBT2BP`の形式である。Excelファイル名は抽出できたオーダー番号をそのまま使用し、例えば`JN24G75406.xlsx`となる。オーダー番号が安全に読めない場合だけ、推測せず`要確認_元PDF名_日時.xlsx`とする。

確認状態は全行確認方式から変更し、正常行は`読取済み・確認不要`、不確かな行だけを`警告あり・警告確認`とする。主文字・L側・R側・行き先が未読取の場合は、右端の`警告・除外理由`に対象項目を明記し、主文字セルは利用者が直接入力できる。図面に実在する電線サイズのシートだけを作成し、サイズ不明・見出し不明の行は`警告一覧`へ集約する。

## 5. 実PDF検証結果

4ページ実PDF `20260824152536.pdf` を使用し、初期進捗 `0/4`、途中 `1/4`、完了 `4/4` を確認した。処理時間は91.9秒、候補行は230行、警告行は79件であった。警告セルの専用画像は79件生成され、正常行とPDF全体が外部支援用フォルダへ入っていないことを確認した。

| 検証項目 | 結果 |
|---|---|
| Python構文検査 | 合格 |
| 業務ルール回帰 | `ALL RULE TESTS PASSED` |
| Excelレイアウト回帰 | `ALL EXCEL LAYOUT TESTS PASSED` |
| 4ページPDF初期進捗 | `0/4` |
| 4ページPDF途中進捗 | `1/4` |
| 4ページPDF完了進捗 | `4/4` |
| 4ページPDF処理時間 | 約91.9秒（開発環境） |
| 4ページPDF候補行 | 230行 |
| 警告行 | 79件 |
| 警告セル専用画像 | 79件 |
| 最新Windowsビルド | 成功 |

主文字については、既に現行のRapidOCR small方式が対象実図面36セルで31/36一致、別集計のYF/ZF-B/IF 37セルで34/37一致しており、600dpi化・単純な罫線除去・大きいOCRモデルへの変更より現行方式が良いことを確認している。局所補正の比較では、CLAHEや軽いシャープ化は現行の31/36を上回らず、適応二値化・Otsu・過剰な罫線マスクは誤読を増やすため標準採用していない。これは「補正を増やせば必ず精度が上がるわけではない」という重要な検証結果である。

## 6. 彩花に確認してほしい判断事項

第一に、外部支援の安全境界をこのまま維持するか確認してほしい。現版では、Copilotへの自動API接続ではなく、警告画像1件の手動添付に限定している。会社側でMicrosoft 365 Copilot API、Copilot Studioエージェント、管理者同意、データ保持規程が正式に確認できた場合のみ、将来のAPI接続を検討する。

第二に、Copilotの回答をExcelへ反映する前に、利用者が「候補」「原図一致」「反映」の3段階で確認するUIを追加する案を確認してほしい。現時点では、誤った外部回答が自動確定されないことを優先している。

第三に、警告セルをクリックするとPDF該当ページ・枠・行を開き、主文字・L側・R側の候補を個別再確認できる画面を次段階として進めるか確認してほしい。この画面が完成すれば、Copilotを使わない会社PCでも人手確認を最小化できる。

## 7. 次の推奨ステップ

短期的には、会社PCで最新ZIPを使い、実際の警告行について「警告画像1件の選択」「Copilot画面の起動」「PDF全体を送信しない運用」を確認する。その後、確認済みの警告画像と正解文字列を蓄積し、専用学習用TSVを作成する。追加学習は開発環境で行い、Windows版には推論モデルだけを同梱する方式が安全である。

直接API接続は、会社の管理者が正式な接続方式とデータ取扱いを承認した後に限定する。承認前に、個人用の認証情報や秘密鍵をWindowsアプリへ埋め込むことは行わない。

## 8. 成果物と保存先

| 成果物 | 保存先 |
|---|---|
| 彩花向け本報告 | `yutakaeng_windows_validation/AYAKA_PROJECT_REPORT.md` |
| 持ち運び版説明書 | `yutakaeng_windows_validation/PORTABLE_README.txt` |
| 検証結果 | `yutakaeng_windows_validation/TEST_RESULTS.md` |
| Copilot調査メモ | `yutakaeng_windows_validation/copilot_integration_notes.md` |
| 最新Windows ZIP | ユーザー提供済み `yutakaeng_brushup_portable.zip`（約205MB、SHA-256: `35471b10b5c1ea707ffb7a508ba9c1792be01529ce8cbaba0f075902201a6809`） |
| 構成説明 | `yutakaeng_windows_validation/PROJECT_STRUCTURE.md` |

## References

[1]: https://learn.microsoft.com/en-us/microsoft-365/copilot/extensibility/copilot-apis-overview Microsoft 365 Copilot APIs Overview, Microsoft Learn
[2]: https://learn.microsoft.com/en-us/microsoft-copilot-studio/image-input-analysis Allow file input from users, Microsoft Learn


## 2026-08-27 写真準拠Excel配置の追加修正

写真の確認により、「主文字の先頭へ見出しを付ける」という表現をExcel上部の先頭行ではなく、主文字文字列へ重複付加する意味と誤解していたことが判明した。実際の要望は、電線サイズシートのグループ先頭へYF、ZF、T1、IFなどの見出しを単独行で置き、その直下へ主文字行を並べる形式である。

このため、主文字は`YF-FL1A11`、`ZF-B-BTB2C6`、`T1-TB2F1A1`のように見出しまたは端子台識別子を1回だけ付加し、盤番号はシート上部の盤番号欄へ移した。正常行はこれまでどおり警告確認を不要とし、未読取行だけを警告として残す。写真準拠の見出し単独行、主文字行、未読取行のマーク個数0、既存業務ルールを回帰試験で確認し、すべて合格した。


## 2026-08-28 線サイズ未記載行の出力修正

利用者から、図面に電線サイズがない行についてもシートを作成してデータを残す要望があった。これまで線サイズが未取得の行は`警告一覧`へ集約されていたが、`線サイズ未記載`という専用シートへ振り分けるよう変更した。対象行は削除せず、主文字・L側接続先・R側接続先・行き先・原図照合情報を出力する。

線サイズ未記載シートの行は`読取状態=警告あり`、`確認状態=警告確認`、`警告・除外理由=線サイズ未記載または未読取`とする。利用者はExcel上で線サイズを入力・修正して必要なサイズのシートへ移動できる。Excel回帰試験では専用シート、見出し単独行、警告状態、警告理由を検証し、既存業務ルール試験と併せて合格した。


## 2026-08-28 盤内線ボタンの追加

PDFドロップ画面に「盤内線」ボタンを追加した。ボタンを使用した場合だけ、端子台側のI表記を含む行を対象として抽出し、通常解析で除外しているI線を現行と同じExcel形式で出力する。通常のPDFドロップ解析では、従来どおり端子台接続先のIを盤内行きとして除外する。I表記の判定は端子台に限定し、コネクター上の単独Iを盤内線と誤認しない。

自動試験で、T1のI表記検出、コネクターのIを盤内線と誤認しないこと、専用モードでIを保持すること、既存のZT LEFT/RIGHT保持を確認した。構文・業務ルール・Excelレイアウト回帰試験はすべて合格した。


## 2026-08-28 Xブロック専用モードの追加

PDFドロップ画面に「Xブロック」ボタンを追加した。ボタンからPDFを選択すると、X1、X2などXで始まる端子台ブロックの枠だけを抽出し、他のYF、T1、IFなどは対象外とする。出力は現行のExcel形式を維持し、電線サイズ別シート、線サイズ未記載、線サイズ判別不明、見出し単独行を使用する。既存の安全除外ルールとZT LEFT/RIGHT保持は変更していない。

総点検で、解析終了後に盤内線・Xブロックの専用ボタンが再表示されない問題を発見し、再表示・再有効化するよう修正した。構文、業務ルール、Excelレイアウト回帰試験で、X1/X2判定、他ブロック除外、盤内線判定、既存業務ルールを確認し、すべて合格した。


## 2026-08-28 専用モードのマーク個数と端子台表示の修正

盤内線およびXブロック専用解析では、対象行のマーク個数を常に2個で出力する仕様へ変更した。主文字未読取や線サイズ未記載の場合も、個数は2個とし、読取状態と警告理由で不確実性を示す。通常解析では安全な主文字が読めた行のみ2個、未読取は0個のままとする。

端子台表示はマーク主文字へ自動連結せず、写真どおり同一グループの先頭へ見出し単独行として表示する。Excel回帰試験では、通常解析の未読取0個、Xブロック専用モードの未読取2個、端子台表示分離を確認し、すべて合格した。


## Xブロック全件・IF盤内線対応

Xブロック専用解析は、X1・X2などのXブロック内にある全マークを対象に変更した。盤内線専用解析は、端子台側のI表記だけでなくIFブロック内の全マークも対象にした。どちらも電線サイズごとにExcelシートを分け、線サイズ未記載・判別不明は専用シートへ分けて出力する。

回帰試験では、XブロックとIFの対象判定、既存の通常解析との境界、専用モードのマーク個数2個、端子台表示の見出し単独行分離を確認し、すべて合格した。


## 2026-09-01 全モード共通のExcel上部情報

利用者提供の写真を基準に、通常解析・盤内線・Xブロックの全モードで、各電線サイズシートのA1へオーダー番号、A2へ先頭ページの盤番号を配置する仕様へ変更した。PDFページが切り替わる場合は、ページごとの盤番号を単独行として挿入し、その下へ見出し・マークを並べる。

オーダー番号と盤番号は、図面右下のタイトル欄をページ単位でOCRする。安全に確定できない場合は推測で補完せず`要確認`とする。4ページ実PDFでは、先頭ページの盤番号候補`7A`を抽出できたが、同PDFの画像品質ではオーダー番号と2ページ目以降の盤番号を安全確定できず`要確認`となった。これは誤った盤情報をExcelへ入れないための安全動作であり、鮮明な実図面で追加確認する。
