...
with tempfile.TemporaryDirectory() as temp_dir:
    ...
    expected_headers = ["マーク主文字", "マーク個数", "解析スクリーンショット", "読取状態", "確認状態", "L側接続先", "R側接続先", "線コード", "見出し", "種別", "PDFページ", "枠ID", "行番号", "警告・除外理由"]
    ...
    check([worksheet.cell(8, column).value for column in range(1, 9)], ["FL1A11", 2, None, "読取済み", "確認不要", "L-A", "R-Y2", "Y2"], "main row below header")
    check([worksheet.cell(9, column).value for column in range(1, 5)], [None, 0, None, "警告あり"], "unread main text receives zero marks")
    ...
    check([size_missing.cell(8, column).value for column in range(1, 6)], ["TB2F1A1", 2, None, "警告あり", "警告確認"], "missing size row remains editable warning")
    check(size_missing.cell(8, 14).value, "線サイズ未記載", "missing size reason is explicit")
    ...
    check([size_unclear.cell(8, column).value for column in range(1, 6)], ["IF1", 2, None, "警告あり", "警告確認"], "unclear size row remains warning")
    check(size_unclear.cell(8, 14).value, "線サイズ判別不明", "unclear size reason is explicit")
    ...
    check([zt_sheet.cell(8, column).value for column in range(1, 9)], ["ZTLINE1", 2, None, "読取済み", "確認不要", "RIGHT-Y5", "LEFT-Y5", "Y5"], "RIGHT and LEFT ZT row is written to dedicated sheet")
