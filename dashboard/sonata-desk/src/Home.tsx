...
const navigation = [
  ...
  { id: "candidates", label: "候補曲比較", icon: BarChart3 },
...
const fallbackReferenceTracks = [
  ...
  },
  {
    ...
    introBass: "84.21%",
    drumsRms: "−62.82 dBFS",
    summary: "ユーザー提供Mainを可逆圧縮して正本登録。既存4ステム合成版との左右チャンネル波形相関は0.998以上で、同一曲として高い整合性を確認。",
    audio: `${rawRoot}music_ai/reference_music/audio/002_reference_main.flac`,
...
const fallbackA1Gates = [
  ...
  { id: "G04", label: "BPM・シーン適合", state: "partial", note: "002の123.05 BPM候補は測定済み。DAWまたは聴取で最終確定が必要" },
  { id: "G05", label: "音圧・ダイナミクス", state: "measured", note: "001・002ともMainの実測済み。最終マスターの聴感確認が必要" },
...
const fallbackLedgerRows = [
  {
    ...
    outcome: "実測完了 / 聴取レビュー待ち",
    note: "001・002とも正式Mainを確認済み。Key・全体構成・聴取記録を保留。",
...
const fallbackPatterns = [
  ...
  { id: "P-S-005", label: "provisional", title: "BPM候補はDAWで確定", body: "002の正式Mainでは123.05 BPM候補を観測。既存ステム推定との差をDAWまたは聴取で確定する。", tone: "bg-[#F8F0E1] text-[#7B5A2D]" },
  ...
  { id: "P-F-003", label: "confirmed", title: "正式Mainと履歴Stem Mixを区別", body: "002のユーザー提供Mainを正本として検証済み。既存Stem Mixは比較・来歴確認用として保持する。", tone: "bg-[#E8F0E9] text-[#244131]" },
];
...
const fallbackEvidenceIntegrity = [
  ...
  { id: "002", state: "verified", label: "VERIFIED / CANONICAL", detail: "ユーザー提供の正式Mainを検証済み。既存Stem Mixは比較・来歴確認用として保持。" },
];

const fallbackReviewQueue = [
  { id: "R1", state: "review", title: "002のKeyと全体構成を確定", detail: "検証済みの正式Mainを聴取し、Keyとセクション構成をタイムコード付きで記録する。", sourcePath: fallbackDecisionBrief.sourcePath },
...
type CandidateTrack = { label: string; duration: string; globalRms: string; introRms: string; introLow: string; fullMixOnset: string };
type CandidateComparison = { id: string; title: string; analysisVersion: string; sourcePath: string; limitation: string; master: CandidateTrack; candidates: CandidateTrack[] };
type SyncData = { sourceDigest: string; references: SyncReference[]; a1: { status: string; gates: SyncGate[] }; ledger: SyncLedger[]; patterns: SyncPattern[]; candidateComparisons?: CandidateComparison[]; decisionBrief?: DecisionBrief; evidenceIntegrity?: EvidenceIntegrity[]; reviewQueue?: ReviewQueueItem[] };
...
export default function Home() {
  ...
  const candidateComparisons = syncData?.candidateComparisons ?? [];
      ...
      <div className="relative z-10 mx-auto grid max-w-[1600px] lg:grid-cols-[236px_minmax(0,1fr)]">
        ...
        <main className="min-w-0 px-5 pb-16 pt-6 sm:px-8 sm:pt-8 lg:px-10 xl:px-14">
          ...
          <section id="references" className="scroll-mt-28 pt-20" aria-labelledby="reference-title"><div className="flex flex-col justify-between gap-6 border-t-2 border-[#1C1A17] pt-5 sm:flex-row sm:items-end"><div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#4F6B5A]">01 / Reference comparison</p><h2 id="reference-title" className="mt-3 font-serif text-[32px] font-semibold tracking-[-0.04em] sm:text-[38px]">001・002を、同じ物差しで見る。</h2></div><SourceLink href={sources.audioLedger}>音源台帳を開く</SourceLink></div><ScoreRuler start="00:00" end="LOOP" note="BPM / Bass Onset / Stem Evidence" />
            <div className="mt-7 grid gap-5 xl:grid-cols-2">{referenceTracks.map((track) => <article key={track.id} className="overflow-hidden border border-[#D8D0C1] bg-[#FBF9F4]"><div className="flex items-start justify-between gap-4 border-b border-[#E1DBCE] p-5 sm:p-6"><div><p className="font-mono text-[10px] tracking-[0.16em] text-[#7A756C]">REFERENCE / {track.id}</p><h3 className="mt-2 font-serif text-[30px] font-semibold tracking-[-0.04em]">Cafe {track.id}</h3><p className="mt-1 text-[11px] text-[#706B62]">{track.sourceType}</p></div><span className={`shrink-0 border px-2.5 py-1 font-mono text-[9px] tracking-[0.08em] ${track.status === "verified" ? "border-[#C9D8CB] bg-[#E8F0E9] text-[#244131]" : "border-[#E5C4B6] bg-[#F7EAE5] text-[#8C4634]"}`}>{track.statusLabel}</span></div><div className="grid grid-cols-2 gap-px bg-[#E5DED0] sm:grid-cols-3">{[["Length", track.duration], ["Format", track.sampleRate], ["Tempo", track.bpm], ["Bass onset", track.bassOnset], ["Intro bass", track.introBass], ["Drums RMS", track.drumsRms]].map(([label, value]) => <div key={label} className="bg-[#F7F4EC] px-4 py-4"><p className="font-mono text-[9px] tracking-[0.13em] text-[#8A8378]">{label}</p><p className="mt-2 text-[12px] font-semibold text-[#332F28]">{value}</p></div>)}</div><div className="p-5 sm:p-6"><p className="text-[12px] leading-6 text-[#676158]">{track.summary}</p><div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#E1DBCE] pt-4"><SourceLink href={track.source}>正解データを読む</SourceLink><SourceLink href={track.audio}>FLACを開く</SourceLink></div></div></article>)}</div><div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_330px]"><p className="self-center font-mono text-[10px] leading-5 text-[#7C756B]">* BPMはアルゴリズム推定値。最終値はDAWまたは聴取で確認する。</p><DecisionNote title="B1は音源差ではなく、導入時刻を比べる。" body="001の約2.3秒と002の約0.3秒。比較以外の条件を固定し、002の検証済み正本Mainを基準に判断する。" action="A1の固定条件を見る" href={sources.a1} /></div></section>

          {candidateComparisons.length > 0 && <section id="candidates" className="scroll-mt-28 pt-20" aria-labelledby="candidates-title"><div className="flex flex-col justify-between gap-6 border-t-2 border-[#1C1A17] pt-5 sm:flex-row sm:items-end"><div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#4F6B5A]">02 / Candidate comparison</p><h2 id="candidates-title" className="mt-3 font-serif text-[32px] font-semibold tracking-[-0.04em] sm:text-[38px]">候補曲を、001の測定値で比べる。</h2></div><p className="max-w-[420px] text-[12px] leading-6 text-[#706B62]">実測値は比較の根拠であり、楽器の存在・主観的な音色・Loop品質の最終判定は別途聴取レビューで確定します。</p></div><div className="mt-7 space-y-5">{candidateComparisons.map((comparison) => <article key={comparison.id} className="overflow-hidden border border-[#D8D0C1] bg-[#FBF9F4]"><div className="flex flex-col justify-between gap-3 border-b border-[#E1DBCE] bg-[#F3F0E8] px-5 py-4 sm:flex-row sm:items-center sm:px-6"><div><p className="font-mono text-[9px] tracking-[0.14em] text-[#7C756B]">ANALYSIS / {comparison.analysisVersion}</p><h3 className="mt-1 text-[15px] font-semibold text-[#292720]">{comparison.title}</h3></div><SourceLink href={`${githubRoot}${comparison.sourcePath}`}>測定JSONを開く</SourceLink></div><div className="grid gap-px bg-[#E5DED0] lg:grid-cols-3"><div className="bg-[#E8F0E9] p-5"><p className="font-mono text-[9px] tracking-[0.13em] text-[#3F5D4B]">CANONICAL / {comparison.master.label}</p><div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 text-[11px] text-[#4C584D]">{[["Length", comparison.master.duration], ["Global RMS", comparison.master.globalRms], ["Intro RMS", comparison.master.introRms], ["Intro low", comparison.master.introLow], ["Full-mix onset", comparison.master.fullMixOnset]].map(([label, value]) => <div key={label}><p className="font-mono text-[8px] tracking-[0.1em] text-[#708071]">{label}</p><p className="mt-1 font-semibold">{value}</p></div>)}</div></div>{comparison.candidates.map((track) => <div key={track.label} className="bg-[#F8F5ED] p-5"><p className="font-mono text-[9px] tracking-[0.13em] text-[#8B6438]">CANDIDATE / {track.label}</p><div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 text-[11px] text-[#625D54]">{[["Length", track.duration], ["Global RMS", track.globalRms], ["Intro RMS", track.introRms], ["Intro low", track.introLow], ["Full-mix onset", track.fullMixOnset]].map(([label, value]) => <div key={label}><p className="font-mono text-[8px] tracking-[0.1em] text-[#8A8378]">{label}</p><p className="mt-1 font-semibold">{value}</p></div>)}</div></div>)}</div><p className="border-t border-[#E1DBCE] px-5 py-3 font-mono text-[9px] leading-5 text-[#7C756B]">LIMITATION: {comparison.limitation}</p></article>)}</div></section>}

          <section id="a1" className="scroll-mt-28 pt-20" aria-labelledby="a1-title"><div className="grid gap-8 border-t-2 border-[#1C1A17] pt-5 xl:grid-cols-[0.8fr_1.2fr]"><div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#4F6B5A]">03 / A1 progress</p><h2 id="a1-title" className="mt-3 font-serif text-[32px] font-semibold tracking-[-0.04em] sm:text-[38px]">確定したことと、<br />まだ聞くべきこと。</h2><ScoreRuler start="G01" end="G09" note="Measured / Pending / Hold" /><p className="mt-5 max-w-[420px] text-[13px] leading-7 text-[#666158]">A1は実測を終え、002の正式Mainも検証済みです。Key・全体構成・テンポ確定・聴取記録など、未観測の値を推測で埋めないことが次の比較の前提です。</p><div className="mt-7 border border-[#E5C4B6] bg-[#F7EAE5] p-5"><div className="flex items-center gap-2 text-[#A45B40]"><CircleAlert className="h-4 w-4" /><span className="font-mono text-[10px] tracking-[0.14em]">CURRENT REVIEW</span></div><p className="mt-3 text-[12px] leading-6 text-[#5D5549]">002の正式Mainは検証済みです。Key、全体構成、テンポ最終値、Loop・聴取記録をタイムコード付きで確定します。</p><div className="mt-4"><SourceLink href={sources.a1}>A1台帳を開く</SourceLink></div></div></div>
